...
const isValidName = process.argv.length === 3;
  ...
  return new Promise(resolve => {
    for (const key in blockContent) {
      fs.writeFile(`${DIR_PATH}${newName}/${newName}.${key}`, blockContent[key], 'utf8', error => {
        if (error) {
          notify('The block wasn\'t created. Something went wrong.');
        }
      });
    }
  });
...
function createBlock() {
  if (isValidName) {
    isNewNameAvailable(newName)
      .then(() => {
        createBlockFolder(newName);
      })
      .then(() => {
        createBlockFiles(newName);
      })
      .then(() => {
        notify(`The block ${newName} was successfully created.`);
      });
  }else {
    ...
    isNewNameAvailable(newName)
      .then(() => {
        ...
        fs.writeFile(`${DIR_PATH}${newName}.pug`, pageContent, 'utf8', error => {
          if (error) {
            notify('The page wasn\'t created. Something went wrong.');
          }
        });
      })
      .then(() => {
        notify(`The page "${newName}" was successfully created.`);
      });
  }else {
